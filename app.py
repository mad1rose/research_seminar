...
def _session_singer_names(num_parts: int) -> list[str]:
    """Ensure session['singer_names'] has exactly num_parts non-empty labels."""
    raw = list(session.get('singer_names') or [])
    out: list[str] = []
    for i in range(num_parts):
        if i < len(raw) and raw[i] is not None and str(raw[i]).strip():
            out.append(str(raw[i]).strip())
        else:
            out.append(f'Singer {i + 1}')
    if raw != out:
        session['singer_names'] = out
...
def profile(index):
    ...
    prev_name = names_list[index - 1] if index > 0 else None
    next_label = names_list[index + 1] if index < num_parts - 1 else None
    is_last = index == num_parts - 1
    profile_page_config = {
        'profileIndex': index,
        'existing': existing or {},
        'saveProfileUrl': url_for('save_profile_api', index=index),
        'navBackUrl': url_for('names') if index == 0 else url_for('profile', index=index - 1),
        'navNextUrl': url_for('summary') if is_last else url_for('profile', index=index + 1),
    }
    ...
    return render_template(
        ...
        is_last=is_last,
        profile_page_config=profile_page_config,
    )
...
def save_profile_api(index):
    ...
    profiles = list(session.get('profiles', []))
    while len(profiles) < num_parts:
